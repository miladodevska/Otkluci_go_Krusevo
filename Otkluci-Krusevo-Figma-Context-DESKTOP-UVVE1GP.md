...
1. ~~**Фонт замена "Playfair Display" → "A1 Serif"**~~ — **РЕШЕНО во React PWA проекта**: корисникот имал "A1 Sans" и "A1 Serif" .ttf фајлови локално (A1Sans-Light/Regular/Medium/Bold, A1Serif-Regular/Bold), копирани во `src/assets/fonts/` и вклучени преку `@font-face` во `src/index.css`. `--font-heading` → A1 Serif (наслови), `--font-body` → A1 Sans (сè останато). Figma самиот фајл сè уште нема "A1 Serif" достапен (само "Gothic A1") — ова е независно решено само во кодот.
